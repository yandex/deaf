...
public class QuickResponsesLoader extends AbstractContentProviderLoader<List<Phrase>> {

    private final PhrasesDAO mPhrasesDao;

    public QuickResponsesLoader(final Context context) {
        super(context, PhrasesDAO.CONTENT_URI);
        mPhrasesDao = new PhrasesDAO(context);
    }

    @Override
    public List<Phrase> loadInBackground() {
        return mPhrasesDao.getQuickResponses();
    }
